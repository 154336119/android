...
public class OrderShowPayActivity extends BaseActivity implements OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_pay:// 支付 - 下个页面
            	...
            	bundle.putString(AppConstants.PARAM_PAY_MONEY,String.valueOf(mPayInfo.getAmount_receivable())); //测试
	...
	private void setData(PayInfo payInfo) {
        ...
        mContractAccount.setText("合同金额："+Util.formatMoneyNoSingle(payInfo.getContract_price(), 2)+"元（已付金额："+Util.formatMoneyNoSingle(payInfo.getReceive(), 0)+"元）");
        mEarnestMoney.setText("预付金额："+Util.formatMoneyNoSingle(payInfo.getOrder_prepay(), 2)+"元");
        mChangeMoney.setText("增加费用："+Util.formatMoneyNoSingle(payInfo.getZj_price(), 2)+"元");
