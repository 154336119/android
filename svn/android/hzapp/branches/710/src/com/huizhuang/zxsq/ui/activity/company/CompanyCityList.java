...
import com.huizhuang.zxsq.widget.AssortView.OnTouchAssortListener;
...
//	}
	...
	private void setDate(List<CityOpen> list) {
		if (list == null) {
			...
			list = getScreeningOpenCity(list); //筛选城市
			setCityMap(list);
			String[] citys = new String[list.size()];
			for (int i = 0; i < citys.length; i++) {
				citys[i] = list.get(i).getName();
			}
			initViews(citys);
		}
	}
	...
	private void setCityMap(List<CityOpen> list){
		for(CityOpen city : list){
			mCityMap.put(city.getName(), city.getId());
		}
	}
	private void initViews(String[] citys) {
//		LinearLayout cityBar = (LinearLayout) findViewById(R.id.ll_mycity_bar);
		mAssortView = (AssortView)findViewById(R.id.assortview);
...
//			});
//		}
		mAssortView.setOnTouchAssortListener(new OnTouchAssortListener() {
			PopupWindow popupWindow;
			int index =-1;
			View layoutView=LayoutInflater.from(CompanyCityList.this)  
                    .inflate(R.layout.popupwindow_display_letter, null); 
			TextView tvDisplayLetter = (TextView)layoutView.findViewById(R.id.tv_popupwindow_display_letter_dispaly_letter);
			@Override
			public void onTouchAssortListener(String s) {
				for(int i=0;i<mAssortView.assort.length;i++){
					if(s.equals(mAssortView.assort[i])){
						index = i;
						break;
					}
				}
				...
				if(popupWindow==null){
					popupWindow = new PopupWindow(layoutView, 300, 300,false);
				}
				tvDisplayLetter.setText(s);
	...
	private List<SortModel> filledData(String[] date) {
		...
		for (int i = 0; i < date.length; i++) {
			...
			if (pinyin.equals("zhongqingshi")||pinyin.equals("zhongqing")) {
				pinyin = "chongqingshi";
			}
	...
	private List<CityOpen> getScreeningOpenCity(List<CityOpen> list){
		List<CityOpen> rList = new ArrayList<CityOpen>();
		for(CityOpen co : list){
			if(co.getLevel() == 2 ){
				rList.add(co);
			}
		}
