...
public class GPUImageFilterTools {  
    ...
    public static FilterList getFilterList(){
    	final FilterList filters = new FilterList();
        filters.addFilter("对比", FilterType.CONTRAST);
        filters.addFilter("反转", FilterType.INVERT);
        filters.addFilter("像素", FilterType.PIXELATION);
        filters.addFilter("色调", FilterType.HUE);
        filters.addFilter("变暗", FilterType.GAMMA);
        filters.addFilter("老照片", FilterType.SEPIA);
        filters.addFilter("灰度", FilterType.GRAYSCALE);
        filters.addFilter("锐化", FilterType.SHARPEN);
        filters.addFilter("线条", FilterType.SOBEL_EDGE_DETECTION);
        filters.addFilter("影像", FilterType.THREE_X_THREE_CONVOLUTION);
        filters.addFilter("浮雕", FilterType.EMBOSS);
        filters.addFilter("色印", FilterType.POSTERIZE);
        filters.addFilter("过滤", FilterType.FILTER_GROUP);
        filters.addFilter("黑白", FilterType.MONOCHROME);
        filters.addFilter("透明", FilterType.OPACITY);
        filters.addFilter("镜头", FilterType.VIGNETTE);
    ...
    public static GPUImageFilter createFilterForType(final Context context, final FilterType type) {
        switch (type) {
            ...
            case BRIGHTNESS:
                return new GPUImageBrightnessFilter(2.0f);
            case GRAYSCALE:
