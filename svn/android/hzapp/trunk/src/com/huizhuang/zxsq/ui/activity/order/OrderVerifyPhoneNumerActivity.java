...
public class OrderVerifyPhoneNumerActivity extends BaseActivity implements OnClickListener {
	...
	private void setTagAliasCallback() {
		mTagAliasCallback = new TagAliasCallback() {
			...
			public void gotResult(int code, String alias, Set<String> tags) {
				String[] aliasAndTag = {alias,AppConfig.JPUSH_TAG};//第一个值表示alias,第二个值表示tag
				switch(code){
				case 0://绑定成功
	...
	private void userLogin(String token,String user_id,String phone) {
		...
		String[] aliasAndTag = {user.getUser_id(),AppConfig.JPUSH_TAG};//第一个值表示alias,第二个值表示tag
		setAliasAndTag(aliasAndTag);//将用户的id上传到JPush服务器，以便JPush针对单位用户推送消息
	} 
     ...
     * 新用户登录得优惠券
     */
    private void httpRequestAddCoupon(final String order_id) {
...
//                    CouponDialog couponDialog = new CouponDialog(OrderVerifyPhoneNumerActivity.this);
//                    couponDialog.builder(coupon.getAmount());
//                    couponDialog.show();
                    showToastMsg("亲，您已成功获得"+coupon.getAmount()+"元惠装优惠券，可在\"我的钱包\"中查看!");
                }
                if(order_id != null){
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            switch (what) {
                ...
                case WHAT_SET_ALIAS://为JPush设置别名
					...
					JPushInterface.setAliasAndTags(OrderVerifyPhoneNumerActivity.this, alias, jpush_tag, mTagAliasCallback);
                default:
                    break;
            }
        }
