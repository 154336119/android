...
public class InfiniteViewPager extends ViewPager {
	...
	public void setAdapter(PagerAdapter adapter) {
		...
		// offset first element so that we can scroll to the mActionBarBackBtn
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int lastHeight = getMeasuredHeight();
		if (wrapHeight && rowHeight == 0) {
			...
			if (getChildCount() > 0) {
				...
				lastChild.measure(widthMeasureSpec, MeasureSpec
						.makeMeasureSpec(height, MeasureSpec.AT_MOST));

				height = firstChild.getMeasuredHeight();
				lastHeight = lastChild.getMeasuredHeight();
				if (!sixWeeksInCalendar && height > lastHeight) {
					height = lastHeight;
				}
