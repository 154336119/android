...
public class Util {
	...
	public static User parseShareSDKUser(String platform,
	 ...
	 * 
	 * @param silent
	 *            是否指定平台分享 true指定平台的一键快捷分享 false有界面的多个平台分享
	 * @param context
	 * @param share
	 *            分享的参数(isCallBack为true时接收回调结果，否则不接收,如果本地图片和网络图片地址都没传就默认截屏)
	 */
	public static void showShare(boolean silent, Activity context, Share share) {
//		AnalyticsUtil.onEvent(context, UmengEvent.ID_SHARE);
//		if (share == null) {
//			share = new Share();
//		}
//		if (!silent) {
...
//					&& TextUtils.isEmpty(share.getImagePath())) {
//				Bitmap bitmap = BitmapFactory.decodeResource(
//						context.getResources(), R.drawable.share);
//				String path = ScreenShotUtil.savePic(bitmap);
	...
	public static String createImagePath(Context context) {
		if (android.os.Environment.getExternalStorageState().equals(
			...
			try {
				...
				e.printStackTrace();
				Toast.makeText(context, "在存储卡中创建图片失败", Toast.LENGTH_LONG)
						.show();
			}
		} else {
			Toast.makeText(context, "存储卡已取出，拍照、相册功能暂不可用", Toast.LENGTH_LONG)
					.show();
		}
		return null;
	}
