 ...
 * <br/>
 * 4) An infinite view pager that allow user to swipe mActionBarBackBtn/right to change
 ...
 * also swipe mActionBarBackBtn/right to change months.<br/>
 * <br/>
...
public class DiaryCalendarFragment extends DialogFragment {
	...
	private HashMap<DateTime, String> diaryDates = new HashMap<DateTime, String>();
	 ...
	 */
	protected ArrayList<DiaryCalendarGridAdapter> datePagerAdapters = new ArrayList<DiaryCalendarGridAdapter>();
	...
	/**
	 * @return the editClickListener
	 */
	public OnClickListener getEditClickListener() {
		return editClickListener;
	}

	/**
	 * @param editClickListener the editClickListener to set
	 */
	 ...
	 */
	public DiaryCalendarGridAdapter getNewDatesGridAdapter(int month, int year) {
		return new DiaryCalendarGridAdapter(getActivity(), month, year,
				getCaldroidData(), extraData, diaryDates);
	}
	 ...
	 * @return
	 */
	public ArrayList<DiaryCalendarGridAdapter> getDatePagerAdapters() {
	...
	/**
	 * @param mSelectDateTime the mSelectDateTime to set
	 */
	public void setSelectDateTime(DateTime mSelectDateTime) {
		this.mSelectDateTime = mSelectDateTime;
	}
	...
	public void restoreDialogStatesFromKey(FragmentManager manager,
		...
		DiaryCalendarFragment existingDialog = (DiaryCalendarFragment) manager
	...
	public void moveToDateTime(DateTime dateTime) {
		...
		// Calendar swipe mActionBarBackBtn when dateTime is in the past
		if (dateTime.lt(firstOfMonth)) {
			// Get next month of dateTime. When swipe mActionBarBackBtn, month will
			// decrease
	...
	public void prevMonth() {
		dateViewPager.setSixWeeksInCalendar(true);
	...
	public void toggle() {
		DiaryCalendarGridAdapter adapter = (DiaryCalendarGridAdapter) fragments
				.get(pageChangeListener.getCurrentPage()
						% DiaryCalendarFragment.NUMBER_OF_PAGES)
				.getGridAdapter();
		adapter.setSelectDateTime(mSelectDateTime);
		if (adapter.getDatetimeList().size() > 7) {
			adapter.oneWeeksInCalendar();
			dateViewPager.setSixWeeksInCalendar(false);
		} else {
			adapter.sixWeeksInCalendar();
			dateViewPager.setSixWeeksInCalendar(true);
		}
		dateViewPager.setDatesInMonth(adapter.getDatetimeList());
	...
	public void refreshView() {
		...
		for (DiaryCalendarGridAdapter adapter : datePagerAdapters) {
			// Reset caldroid data
			adapter.setCaldroidData(getCaldroidData());

			// Reset extra data
			adapter.setExtraData(extraData);
			adapter.setDiaryDates(diaryDates);
			adapter.setSelectDateTime(mSelectDateTime);
			// Refresh view
			adapter.notifyDataSetChanged();
		}
	 ...
	 * @return
	 */
	public static DiaryCalendarFragment newInstance(String dialogTitle, int month,
			int year) {
		DiaryCalendarFragment f = new DiaryCalendarFragment();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// Inflate layout
		View view = inflater.inflate(R.layout.diary_calendar_view, container, false);

		// For the monthTitleTextView
		monthTitleTextView = (TextView) view.findViewById(R.id.calendar_month_year_textview);

		mTodaButton = (Button) view.findViewById(R.id.calendar_today);
		mTodaButton.setOnClickListener(new OnClickListener() {
		...
		mEditButton = (ImageButton) view.findViewById(R.id.diary_edit);
		...
		// For the mActionBarBackBtn arrow button
...
//				getDaysOfWeek());
		WeekdayAdapter weekdaysAdapter=new WeekdayAdapter(getActivity(), getDaysOfWeek());
	...
	private void setupDateGridPages(View view) {
		...
		// Current month
		DiaryCalendarGridAdapter adapter0 = getNewDatesGridAdapter(
		...
		for (int i = 0; i < NUMBER_OF_PAGES; i++) {
			...
			DiaryCalendarGridAdapter adapter = datePagerAdapters.get(i);
	...
	public class DatePageChangeListener implements OnPageChangeListener {
		...
		private ArrayList<DiaryCalendarGridAdapter> caldroidGridAdapters;
		 ...
		 * @return
		 */
		public ArrayList<DiaryCalendarGridAdapter> getCaldroidGridAdapters() {
			return caldroidGridAdapters;
		}

		public void setCaldroidGridAdapters(
				ArrayList<DiaryCalendarGridAdapter> caldroidGridAdapters) {
		...
		private int getNext(int position) {
			return (position + 1) % DiaryCalendarFragment.NUMBER_OF_PAGES;
		}
		...
		private int getPrevious(int position) {
			return (position + 3) % DiaryCalendarFragment.NUMBER_OF_PAGES;
		}
		...
		public int getCurrent(int position) {
			return position % DiaryCalendarFragment.NUMBER_OF_PAGES;
		}
		...
		public void refreshAdapters(int position) {
			...
			DiaryCalendarGridAdapter currentAdapter = caldroidGridAdapters
					.get(getCurrent(position));
			DiaryCalendarGridAdapter prevAdapter = caldroidGridAdapters
					.get(getPrevious(position));
			DiaryCalendarGridAdapter nextAdapter = caldroidGridAdapters
			...
			// Detect if swipe right or swipe mActionBarBackBtn
			// Swipe right
		...
		public void onPageSelected(int position) {
			...
			DiaryCalendarGridAdapter currentAdapter = caldroidGridAdapters.get(position % DiaryCalendarFragment.NUMBER_OF_PAGES);
			dateViewPager.setDatesInMonth(currentAdapter.getDatetimeList());
