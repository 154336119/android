...
public class JpushReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(action)) {// 接收到注册的ID
			...
			LogUtil.i("[JpushReceiver] 接收Registration Id : " + regId);
			...
			LogUtil.i("[JpushReceiver] 接收到推送下来的自定义消息: " + customMessage);
			// 对自定义的消息进行处理
		} else if (JPushInterface.ACTION_NOTIFICATION_RECEIVED.equals(action)) {// 接收到通知
			LogUtil.i("[JpushReceiver] 接收到推送下来的通知");
			int notifactionId = bundle.getInt(JPushInterface.EXTRA_NOTIFICATION_ID);
			LogUtil.i("[JpushReceiver] 接收到推送下来的通知的ID: " + notifactionId);
		} else if (JPushInterface.ACTION_NOTIFICATION_OPENED.equals(action)) {// 打开通知
			LogUtil.i("[JpushReceiver] 用户打开了通知");
